...
# # 2. 이미지 분석 코어 로직 (동기)
# # ==========================================
# def process_core_logic_sync(img_cv2: np.ndarray, mode: str = "full") -> str:
#     """
#     mode: "full" (전체 분석), "collision" (위험 감지), "count" (객체 카운트)
#     """
#     if detector is None:
...
# async def fetch_image_async(image_url: str) -> np.ndarray:
#     """aiohttp를 활용한 비동기 이미지 다운로드 (타임아웃 적용)"""
#     direct_url = convert_gdrive_url(image_url)
    
#     # p99 3000ms 방어를 위한 2.5초 타임아웃
...
#         if img_cv2 is None:
#             raise ValueError("URL에서 유효한 이미지를 읽을 수 없습니다.")
#         return img_cv2
        
#     except asyncio.TimeoutError:
#         raise Exception("네트워크 지연으로 이미지를 불러올 수 없습니다. (타임아웃)")
#     except Exception as e:
...
#                     "description": f"Provides a comprehensive visual analysis and obstacle report from a given public URL. Provided by {SERVICE_NAME}.",
#                     "inputSchema": {
#                         "type": "object",
...
#                 },
#                 {
#                     "name": "check_collision_danger",
#                     "description": f"Quickly checks for immediate collision dangers (obstacles taking >50% of view) from a given URL. Provided by {SERVICE_NAME}.",
#                     "inputSchema": {
#                         "type": "object",
#                         "properties": {"image_url": {"type": "string", "description": "Public URL of the image"}},
#                         "required": ["image_url"]
...
#                 },
#                 {
#                     "name": "count_detected_objects",
#                     "description": f"Counts and lists all detected objects from a given URL without positional details. Provided by {SERVICE_NAME}.",
#                     "inputSchema": {
#                         "type": "object",
#                         "properties": {"image_url": {"type": "string", "description": "Public URL of the image"}},
#                         "required": ["image_url"]
...
#             "result": {"content": [{"type": "text", "text": "⚠️ 오류: 이미지 URL이 입력되지 않았습니다."}]}
#         }

#     try:
#         # 1. 비동기로 이미지 다운로드 (이벤트 루프 차단 없음)
#         img_cv2 = await fetch_image_async(image_url)
        
#         # 2. 툴 이름에 따라 모드(mode) 설정
#         if tool_name == "analyze_vision_url":
#             mode = "full"
#         elif tool_name == "check_collision_danger":
#             mode = "collision"
#         elif tool_name == "count_detected_objects":
#             mode = "count"
#         else:
#             return {
#                 "jsonrpc": "2.0", "id": req_id,
#                 "result": {"content": [{"type": "text", "text": f"Error: 알 수 없는 툴 이름입니다. ({tool_name})"}]}
#             }

#         # 3. 모델 추론은 동기 함수이므로 별도의 쓰레드에서 실행
#         text_content = await asyncio.to_thread(process_core_logic_sync, img_cv2, mode)

#     except Exception as e:
#         text_content = f"⚠️ 처리 중 오류가 발생했습니다: {str(e)}"
...
# # ==========================================
# # 4. 엔드포인트 라우팅 (기존 SSE 연결 로직 완벽 유지)
# # ==========================================
# @app.get("/mcp")
