...
def fetch_and_analyze_sync(image_url: str, mode: str) -> str:
    """이미지 다운로드 및 MediaPipe 분석 통합 로직 (쓰레드 내부 실행용)"""
    # 1. 이미지 다운로드 (p99 3000ms 규정 방어를 위한 2.5초 타임아웃 필수)
    try:
        ...
        return "⚠️ 오류: 이미지 다운로드 시간이 초과되었습니다. (서버 응답 지연)"
    except Exception as e:
        return f"⚠️ 이미지 다운로드 실패: {str(e)}"
    ...
    # mode == "full"
    result_text = f"### 👁️ {SERVICE_NAME} 분석 보고서\n\n"
    if collision_warnings:
...
async def handle_tools_call(req_id: str | int, params: dict) -> dict:
    ...
    # 툴 이름에 따라 mode 매핑
    mode_map = {
        "analyze_vision_url": "full",
        "check_collision_danger": "collision",
        "count_detected_objects": "count"
    }
    
    if tool_name not in mode_map:
        return {
            "jsonrpc": "2.0", "id": req_id,
            "result": {"content": [{"type": "text", "text": f"Error: 알 수 없는 툴 이름입니다. ({tool_name})"}]}
        }

    mode = mode_map[tool_name]

    try:
        # 동기 방식의 이미지 다운로드 및 처리 로직을 비동기 이벤트 루프 밖(별도 쓰레드)에서 실행
        # 이를 통해 하나의 요청이 2~3초 걸리더라도 다른 사용자의 API 요청(health check 등)이 멈추지 않습니다.
        text_content = await asyncio.to_thread(fetch_and_analyze_sync, image_url, mode)
    except Exception as e:
